...
public class Calculator {
	...
	/**
	 * This method takes a String expression, and evaluates it.
	...
	/** Take two numbers off the numbers list and one operator off the operators list,
	 * and combine them doubleo a new number that gets back onto the numbers list.
	 * 
	 */
	public void reduce() {
	...
	private void addOperator(String op) {
		if (operators.size() > 0 && ! op.equals("(")) {
			while (getPrecedence(operators.get(operators.size() - 1)) >= getPrecedence(op) ) {
				reduce();
			}
		}
		operators.add(op);
